...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions
{
    ...
    controller.importIsNeeded = self.importIsNeeded;
    controller.managedObjectContext = self.managedObjectContext;
...
- (void)initStorage {
    ...
    url = [url URLByAppendingPathComponent:@"GuessTheArtist"];
    ...
    if (![[NSFileManager defaultManager] fileExistsAtPath:[url path]]) {
        [document saveToURL:url
          ...
          completionHandler:^(BOOL success) {
              if (success) {
                  ...
                  GTAViewController *viewController = (GTAViewController *)self.window.rootViewController;
                  viewController.managedObjectContext = self.managedObjectContext;
                  viewController.importIsNeeded = true;
                  [[NSNotificationCenter defaultCenter] postNotificationName:@"DatabaseReady"
                                                                      object:self];
              }
          }];
        ...
        [document openWithCompletionHandler:^(BOOL success) {
            if (success) {
                ...
                GTAViewController *viewController = (GTAViewController *)self.window.rootViewController;
                viewController.managedObjectContext =  self.managedObjectContext;
                viewController.importIsNeeded = false;
                [[NSNotificationCenter defaultCenter] postNotificationName:@"DatabaseReady"
                                                                    object:self];
            }
        }];
        ...
        GTAViewController *viewController = (GTAViewController *)self.window.rootViewController;
        viewController.managedObjectContext = self.managedObjectContext;
        viewController.managedObjectContext = document.managedObjectContext;
        viewController.importIsNeeded = false;
        [[NSNotificationCenter defaultCenter] postNotificationName:@"DatabaseReady"
                                                            object:self];
    }
...
- (NSPersistentStoreCoordinator *)persistentStoreCoordinator
{
    ...
    NSURL *storeURL = [[self applicationDocumentsDirectory] URLByAppendingPathComponent:@"Paintings.sqlite"];
